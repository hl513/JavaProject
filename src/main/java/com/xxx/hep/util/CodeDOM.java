...
public class CodeDOM {
    ...
    private CodeDOM(String tableName) {
        ...
        basePath = System.getProperty("user.dir") + "\\jpa06\\src\\main\\java\\" + package_;
    }
    ...
    private void createPojo(List<TableInfo> tableInfos) {
        String className = StringUtil.captureName(StringUtil.camelCaseName(tableName));
        File file = FileUtil.createFile(basePath + "pojo" + File.separator + className + ".java");

        StringBuffer stringBuffer = new StringBuffer();
        stringBuffer.append("package ").append(package_.replaceAll("\\\\", ".")).append("pojo;\n\n")
        ...
        for (TableInfo tableInfo : tableInfos) {
            String fieldName = StringUtil.camelCaseName(tableInfo.getColumnName());
            if (fieldNames.contains(fieldName)) continue;
            fieldNames.add(fieldName);

            if ("PRI".equalsIgnoreCase(tableInfo.getColumnKey())) {
                stringBuffer.append("    @Id\n");
            }
            if ("auto_increment".equalsIgnoreCase(tableInfo.getExtra())) {
                stringBuffer.append("    @GeneratedValue(strategy = GenerationType.IDENTITY)\n");
            }

            String javaType = StringUtil.typeMapping(tableInfo.getDataType());
            if (javaType == null) javaType = "String"; // fallback
            stringBuffer.append("    private ").append(javaType).append(" ").append(fieldName).append(";");

            String comment = tableInfo.getColumnComment();
    ...
    private void createRepository(List<TableInfo> tableInfos) {
        ...
        stringBuffer.append(
                "package " + package_.replaceAll("\\\\", ".") + "repository;\n" +
                        "\n" +
                        "import " + basePackage_.replaceAll("\\\\", ".") + "*;\n" +
                        "import " + package_.replaceAll("\\\\", ".") + "pojo." + StringUtil.captureName(StringUtil.camelCaseName(tableName)) + ";\n" +
                        "import org.springframework.stereotype.Repository;\n" +
                        "\n" +
                        "@Repository\n" +
                        "public interface " + StringUtil.captureName(StringUtil.camelCaseName(tableName)) + "Repository extends JpaRepository<" + StringUtil.captureName(StringUtil.camelCaseName(tableName)) + ", " + t + "> {"
        );
        stringBuffer.append("\n");
        stringBuffer.append("}");
        FileUtil.fileWriter(file, stringBuffer);
